...
readonly log_file=./access-4560-644067.log
...
lock_file=/var/tmp/script_hw7_lock
if [ -f $lock_file ]; then
...
if [[ "$remembered_last_line" == "" ]]; then
    ...
    # найдём сохранённую строку и начнём обработку с неё
...
do
    # если начальной даты временнoго диапазона нашего отчета мы пока не имеем, то берём дату первой обрабатываемой строки,
    if [[ "$report_date" == "" ]]; then
        report_date=`extract_date "$line"`
    fi

    http_code=`extract_http_code "$line"`
    if [[ "$http_code" -gt 499 ]]; then errors+=("$line") 
    fi
...
# снимаем защиту от мультизапуска
rm -f $lock_file
trap - INT TERM EXIT